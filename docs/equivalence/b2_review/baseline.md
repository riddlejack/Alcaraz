...
`uv run pytest -q tests/test_barrier_gate.py tests/test_config.py --basetemp <BASELINE_REVIEW_WORKSPACE>` exited 0: 21 tests passed. This reran the original clean synthetic chain, declared-access positives, four-format metric writer negatives, undeclared reader, fold-receipt negatives, forged criterion/non-argmin negatives and existing symlink path tests. The uncovered bugs above demonstrate gaps in those existing tests, rather than failures of their advertised individual controls.
