...
class _DecayingOverall:
    ...
    def apply_batch(self, rows: Sequence[HistoryMatch], major_codes: frozenset[str]) -> None:
        ...
        for row in rows:
            ...
            multiplier = MAJOR_MULTIPLIER if self.major_multiplier and row.level in major_codes else 1.0
            weight = 1.0
            if self.weighted:
...
class RatingSuite:
    ...
    def probabilities(self, target: TargetMatch) -> dict[str, float]:
        return {
            "k32_pooled": self.k32.probability(
                target.player_a, target.player_b, target.surface
            ),
            "kovalchik_overall_major": self.kovalchik.probability(
...
def rating_forecasts(
    ...
    for target in ordered_targets:
        while cursor < len(ordered_history) and ordered_history[cursor].match_date <= target.eligible_through_date:
