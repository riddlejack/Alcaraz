...
independent review rejected that follow-up at `4295df4` because current-version
settlement trusted a coherently rehashed latest pointer before reconciling an unchanged
qualification digest. `docs/live/REPAIR3.md` and `docs/live/REPAIR3_FREEZE.json` freeze
the single-invariant resolver repair; `configs/live/live.json` binds all three repair
contracts. Builder verification is not independent acceptance; the third repair remains
...
explicit and default settlement versions reconcile prior qualification digests before a
matching current pointer, refuse disagreement or multiple anchors, and still admit an
unqualified current version through its pointer;
...
forecast; every other rung writes an explicit unavailable record (see RB18–RB21).
