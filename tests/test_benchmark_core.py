...
def test_equations_and_first_update() -> None:
    ...
    assert math.isclose(suite.welo.ratings["1"], 1500.0 + ordinary * winner_game_share("6-4 6-4"))
...
def test_major_is_kovalchik_only_and_surface_blend_is_distinct() -> None:
    ...
    expected_rating = (
        0.71 * major.fivethirtyeight.overall["1"]
        + 0.29 * major.fivethirtyeight.surface[("1", "Hard")]
    )
...
def test_swap_complementarity() -> None:
    ...
    assert math.isclose(
        p + elo_probability(suite.kovalchik.ratings["2"], suite.kovalchik.ratings["1"]), 1.0
    )
