"""Freeze one ATP walk-forward attempt: bind inputs, code and image, write authorization.

The authorization drives the unchanged ``runtime/empirical_controller.py``. Everything the
controller may read is bound by SHA-256, and the immutable image must contain exactly the
checked adapter bytes. This step runs no source-derived numerical code on real inputs.
"""
...
def main() -> None:
    ...
    if info["status"] != "metadata_only_no_candidate_scores":
        raise ValueError("preparation status mismatch")
    ...
    files = set(native) | {ranking}
    files.update(Path(p) for p in info["output_bindings"])
    files.add(prep / "preparation.json")
    files.add(Path(info["sources"]["frozen_plan"]["path"]))
    files.add(Path(info["sources"]["cohort_carrier"]["path"]))
    files.add(Path(info["sources"]["alcaraz_selected"]["path"]))
    files.add(Path(info["sources"]["selection_json"]["path"]))
    files.update(
        p
        for p in (HERE.parent).rglob("*")
        if p.is_file()
        and "__pycache__" not in p.parts
        and p.suffix in {".py", ".adapter", ".runtime", ".buildoak", ".md"}
    )
