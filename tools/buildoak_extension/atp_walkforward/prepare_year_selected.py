"""Metadata-only preparation of one ATP walk-forward target year, cohort from a selection.

``prepare_year.py`` with one change: the target cohort is the accepted selection's primary
membership read from the selected forecast file itself (``selected/<Y>/hgb/<bundle>.csv`` and
its selection JSON), not from the G-L carrier, which exists only through 2024. The selection
must have no provisional rows (all selected keys are primary targets, proved by the WTA01
``key_hash`` over sorted ``(season, match_id)`` pairs), its cutoff must equal the fit boundary
and its file hash must equal the selection's. Everything else (plan roles, memberships, IOC
vocabulary, replay projection, receipt fields) is ``prepare_year.py`` unchanged, so years whose
selections have no provisional rows reproduce the accepted preparation outputs. No feature
construction, estimator fit or outcome scoring happens here; no labels file is read.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--selected", type=Path, required=True, help="selected/<Y>/hgb/<bundle>.csv"
    )
    ...
    # Target cohort: the accepted selection's primary membership, read from the selected file.
    with args.selected.open(newline="") as handle:
        selected_rows = list(csv.DictReader(handle))
    selected_ids = [r["match_id"] for r in selected_rows]
    if len(selected_ids) != len(set(selected_ids)) or not selected_ids:
        raise ValueError("selected forecast keys are empty or not unique")
    if any(r["season"] != str(year) for r in selected_rows):
        raise ValueError("selected forecast row outside the target season")
    selection = json.loads(args.selection_json.read_text())
    if selection.get("outer_year") != year:
        raise ValueError("selection is for another year")
    if selection.get("outer_provisional_rows") != 0:
        raise ValueError(
            "selection has provisional rows; this variant reads no labels to drop them"
        )
    if selection.get("outer_primary_rows") != len(selected_ids):
    ...
    # WTA01/TIER01 pipeline key_hash over sorted (season, match_id) pairs.
    key_payload = "".join(f"{year},{key}\n" for key in sorted(selected_ids)).encode()
    ...
    receipt = {
        ...
        "cohort_checks": {
            "source": "selected forecast file (all rows primary; no provisional rows)",
        ...
        "sources": {
            ...
            # freeze_year.py binds these two names; both are the selected forecast file here.
            "cohort_carrier": {"path": str(args.selected.resolve()), "sha256": sha(args.selected)},
            "alcaraz_selected": {
                "path": str(args.selected.resolve()),
                "sha256": sha(args.selected),
            },
            "selection_json": {
