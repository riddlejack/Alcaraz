...
def feature_engine(reference_date, ioc_buckets, rankings_index, config=None):
    """D97 coroutine: consume one released history or neutral target at a time.

    Native formula body retained; only history messages apply result updates.
    The caller must qualify and canonically order releases before sending them.
    """
    if config is None:
    ...
    latest_match_date = pd.Timestamp(reference_date)


    event = yield None
    while event is not None:
        kind, source_row = event
        if kind not in {"history", "target"}:
            raise ValueError("unknown event kind")
        series = pd.Series(source_row)
